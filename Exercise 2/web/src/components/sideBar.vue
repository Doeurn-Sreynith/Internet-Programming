    ...
    <main v-if="data">
        ...
        <div class="category" @click="toggle($event)" v-for="item in data" :key="item._id">
            ...
            <div class="sub_category" >
                <!-- {{ item._id }} -->
                <li ><a href="#" class="sub_category_item"  >All</a></li>
                <li v-for="subItem in item.item" :key="subItem._id"><a href="#" class="sub_category_item"  >{{ subItem.name }}</a></li>
            </div>
        </div>
...
fetch(`${store.state.apiUrl}/category/item`)
.then(res=>res.json())
.then(d=>{
    data.value=d.data
})
...
const listAll=()=>{
    isAll.value=true
    store.state.loding=true
    fetch(`${store.state.apiUrl}/item/product`)
    .then(res=>res.json())
    .then(d=>{
        const newdata=[]
        d.data.forEach(item=>{
            newdata.push(...item.product)
        })
        store.state.loding=false
